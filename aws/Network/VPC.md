...
- Virtual Private Cloud:
  ...
  - Subnet:                   VPC内のIPアドレス範囲を分割した単位 / ==AZをまたげない==
    ...
    - `Private Subnet`:         ==NAT 経由で外向き通信 (アウトバウンド) / 公開には ALB/NLB をパブリックに配置==
  ...
  - ファイアウォール:
    - `Network ACL`:            ==Deny== / ==サブネット単位==   / ==ステートレス== / 特定 IP のブロック / エフェメラルポート許可設定
    - `Security Group`:         ==Allow== / ==インスタンス単位== / ==ステートフル== (戻りトラフィックの自動許可)
  - `Internet Gateway (IGW)`:   VPCとインターネット間の通信 / ==高可用性 (Managed)== / 1VPCに1IGW / 
  - `NAT Gateway`:              ==!$0.062/h+$0.062/GB== / ==高可用性 (Managed 単一AZ内)== / Elastic IPが必要 / 複数AZ可能 / ==Outbound専用==
  ...
  - 接続:
    ...
    - `VPC Endpoint`:           インターネットを経由せずにAWSサービスにプライベート接続
      ...
      - `AWS PrivateLink`:      ==VPC 間で特定サービスだけ接続==
        - サービス提供側:     ==NLB 必須== + VPCエンドポイントサービス
